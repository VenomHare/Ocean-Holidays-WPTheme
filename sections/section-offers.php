    ...
    $args = array(
        ...
        'meta_query'     => array(
            array(
                ...
                'value'   => 'featured',             // The value to check (the 'upcoming' tag)
    ...
    if ($tour_query->have_posts()) {
        while ($tour_query->have_posts()) {
            ...
            echo '<div class="w-[35%] h-[80%] blue_shadow bg-white rounded-2xl flex flex-col items-center gap-4 py-4 max-lg:w-[45%] max-sm:w-[95%]">';
                echo "<div class='relative w-[90%] h-[60%]'>";
                    echo '<img class="w-full h-full object-cover rounded-xl relative" src="'.get_the_post_thumbnail_url().'" alt="'.get_the_title().'" />';
                    echo "<div class='absolute text-white  bg-primary font-bold font-poppin p-2 rounded top-2 left-4'>HOTSELLER</div>";
                echo "</div>";
